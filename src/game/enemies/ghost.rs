...
fn init_ghost(
    ...
    for (entity, mut transform) in query.iter_mut() {
        ...
        command.insert((
            ...
            AnimationConfig::new(0, 2, 4, true, true),
            Directional {
                flipdir: true,
                ..Default::default()
            },
...
fn animate_ghost(
    ...
    for (mut ghost, mut animconf, mut sprite, entity, global) in query.iter_mut() {
                ...
                if map.index == 2 && ghost.attackcooldown.finished() {
                    if let Ok(player) = player_query.single() {
                        ...
                        commands.spawn((
                            ...
                            AnimationConfig::new(0, 8, 4, true, true),
                            Directional {
                                flipdir: true,
                                ..Default::default()
                            },
        ...
        ghost.dirty = false;
        match ghost.state {
            State::Roaming => sprite.color = Color::srgb(1.0, 0.0, 0.0),
            State::Aggro => sprite.color = Color::srgb(0.0, 1.0, 0.0),
            State::Attacking => sprite.color = Color::srgb(0.0, 0.0, 1.0),
        }
        match ghost.state {
